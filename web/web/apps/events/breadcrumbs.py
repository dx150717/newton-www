...
def year_crumb(creation_date):
    ...
    return Crumb(year, reverse('events:entry_archive_year',
                               args=[year]))
...
def month_crumb(creation_date):
    ...
    return Crumb(month_text, reverse('events:entry_archive_month',
                                     args=[year, month]))
...
def day_crumb(creation_date):
    ...
    return Crumb(day, reverse('events:entry_archive_day',
                              args=[year, month, day]))
...
ZINNIA_ROOT_URL = lambda: reverse('events:entry_archive_index')

MODEL_BREADCRUMBS = {'Tag': lambda x: [Crumb(_('Tags'),
                                             reverse('events:tag_list')),
                                       Crumb(x.name)],
                     'Author': lambda x: [Crumb(_('Authors'),
                                                reverse('events:author_list')),
                                          Crumb(x.__str__())],
                     'Category': lambda x: [Crumb(
                         _('Categories'), reverse('events:category_list'))] +
