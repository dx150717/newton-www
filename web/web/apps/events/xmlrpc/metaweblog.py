...
from events.managers import DRAFT, PUBLISHED
...
def category_structure(category, site):
    ...
    return {'description': category.title,
            ...
            'rssUrl': '%s://%s%s' % (
                PROTOCOL, site.domain,
                reverse('events:category_feed', args=[category.tree_path])),
...
def tag_structure(tag, site):
    ...
    return {'tag_id': tag.pk,
            ...
            'html_url': '%s://%s%s' % (
                PROTOCOL, site.domain,
                reverse('events:tag_detail', args=[tag.name])),
            'rss_url': '%s://%s%s' % (
                PROTOCOL, site.domain,
                reverse('events:tag_feed', args=[tag.name]))
            }
...
def delete_post(apikey, post_id, username, password, publish):
    ...
    => boolean
    """
    user = authenticate(username, password, 'events.delete_entry')
...
def new_category(blog_id, username, password, category_struct):
    ...
    => category_id
    """
    authenticate(username, password, 'events.add_category')
...
def new_post(blog_id, username, password, post, publish):
    ...
    => post_id
    """
    user = authenticate(username, password, 'events.add_entry')
    ...
    if user.has_perm('events.can_change_status'):
        entry_dict['status'] = publish and PUBLISHED or DRAFT

    entry = Entry.objects.create(**entry_dict)

    author = user
    if 'wp_author_id' in post and user.has_perm('events.can_change_author'):
...
def edit_post(post_id, username, password, post, publish):
    ...
    => boolean
    """
    user = authenticate(username, password, 'events.change_entry')
    entry = Entry.objects.get(id=post_id, authors=user)
    if post.get('dateCreated'):
        creation_date = datetime.strptime(
            post['dateCreated'].value[:18], '%Y-%m-%dT%H:%M:%S')
