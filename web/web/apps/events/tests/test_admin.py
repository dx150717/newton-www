...
from events.admin.entry import EntryAdmin
from events.admin.category import CategoryAdmin
from events.signals import disconnect_entry_signals
from events.url_shortener.backends.default import base36


class BaseAdminTestCase(TestCase):
    rich_urls = 'events.tests.implementations.urls.default'
    poor_urls = 'events.tests.implementations.urls.poor'
    urls = rich_urls
    model_class = None
    admin_class = None
...
class EntryAdminTestCase(BaseAdminTestCase):
    ...
    def test_get_tags(self):
        self.check_with_rich_and_poor_urls(
            ...
            '', '')
        self.entry.tags = 'events'
        self.check_with_rich_and_poor_urls(
            self.admin.get_tags, (self.entry,),
            '<a href="/tags/zinnia/" target="blank">zinnia</a>',
            'events')
        self.entry.tags = 'events, test'
        self.check_with_rich_and_poor_urls(
            self.admin.get_tags, (self.entry,),
            '<a href="/tags/test/" target="blank">test</a>, '
            '<a href="/tags/zinnia/" target="blank">zinnia</a>',
            'events, test')  # Yes, this is not the same order...
