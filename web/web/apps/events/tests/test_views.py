...
from events.views import quick_entry
from events.managers import DRAFT
from events.managers import PUBLISHED
from events.settings import PAGINATION
from events.tests.utils import datetime
from events.tests.utils import urlEqual
from events.flags import user_flagger_
from events.flags import get_user_flagger
...
from events.url_shortener.backends.default import base36
...
class ViewsTestCase(ViewsBaseCase):
    ...
    """
    urls = 'events.tests.implementations.urls.default'

    @override_settings(USE_TZ=False)
    def test_zinnia_entry_archive_index_no_timezone(self):
        template_name_today = 'events/archives/%s/entry_archive.html' % \
    ...
    def test_zinnia_entry_archive_year_no_timezone(self):
        self.inhibit_templates(
            'events/archives/2010/entry_archive_year.html',
            'events/entry_archive_year.html')
        response = self.check_publishing_context(
            '/2010/', 2, 3, 'entry_list', 3)
        self.assertTemplateUsed(
            response, 'events/archives/2010/entry_archive_year.html')
    ...
    def test_zinnia_entry_archive_week_no_timezone(self):
        self.inhibit_templates(
            'events/archives/2010/week/00/entry_archive_week.html',
            'events/entry_archive_week.html')
        response = self.check_publishing_context(
            '/2010/week/00/', 1, 2, 'entry_list', 3)
        self.assertTemplateUsed(
            response, 'events/archives/2010/week/00/entry_archive_week.html')
    ...
    def test_zinnia_entry_archive_month_no_timezone(self):
        self.inhibit_templates(
            'events/archives/2010/month/01/entry_archive_month.html',
            'events/entry_archive_month.html')
        response = self.check_publishing_context(
            '/2010/01/', 1, 2, 'entry_list', 3)
        self.assertTemplateUsed(
            response, 'events/archives/2010/month/01/entry_archive_month.html')
    ...
    def test_zinnia_entry_archive_day_no_timezone(self):
        self.inhibit_templates(
            'events/archives/2010/01/01/entry_archive_day.html',
            'events/entry_archive_day.html')
        response = self.check_publishing_context(
            '/2010/01/01/', 1, 2, 'entry_list', 2)
        self.assertTemplateUsed(
            response, 'events/archives/2010/01/01/entry_archive_day.html')
        self.assertEqual(response.context['previous_month'], None)
        self.assertEqual(response.context['next_month'], date(2010, 5, 1))
    ...
    def test_zinnia_entry_archive_day_with_timezone(self):
        self.inhibit_templates(
            'events/archives/2010/01/02/entry_archive_day.html',
            'events/entry_archive_day.html')
        response = self.check_publishing_context(
            '/2010/01/02/', 1, 2, 'entry_list', 2)
        self.assertTemplateUsed(
            response, 'events/archives/2010/01/02/entry_archive_day.html')
        self.assertEqual(response.context['previous_month'], None)
        self.assertEqual(response.context['next_month'], date(2010, 6, 1))
        self.assertEqual(response.context['previous_day'], None)
    ...
    def test_zinnia_entry_archive_today_no_timezone(self):
        self.inhibit_templates('events/entry_archive_today.html')
        with self.assertNumQueries(2):
            response = self.client.get('/today/')
        self.assertTemplateUsed(response, 'events/entry_archive_today.html')
    ...
    def test_zinnia_entry_detail(self):
        self.inhibit_templates('events/_entry_detail.html', '404.html')
        ...
        self.assertTemplateUsed(response, 'events/_entry_detail.html')

    def test_zinnia_entry_detail_login(self):
        self.inhibit_templates('events/entry_detail.html',
                               'events/login.html')
        entry = self.create_published_entry()
        ...
        response = self.client.post(entry.get_absolute_url(),
                                    {'username': 'admin',
                                     'password': 'password'})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'events/entry_detail.html')

    def test_zinnia_entry_detail_password(self):
        self.inhibit_templates('events/entry_detail.html',
                               'events/password.html')
        entry = self.create_published_entry()
        entry.password = 'password'
        entry.save()
        with self.assertNumQueries(1):
            response = self.client.get(entry.get_absolute_url())
        self.assertTemplateUsed(response, 'events/password.html')
    ...
    def test_zinnia_entry_detail_preview(self):
        self.inhibit_templates('events/entry_detail.html', '404.html')
    ...
    def test_zinnia_entry_channel(self):
        self.inhibit_templates('events/entry_list.html')
        self.check_publishing_context(
            '/channel-test/', 2, 3, 'entry_list', 1)

    def test_zinnia_category_list(self):
        self.inhibit_templates('events/category_list.html')
    ...
    def test_zinnia_category_detail(self):
        self.inhibit_templates('events/category/tests/entry_list.html')
        response = self.check_publishing_context(
            '/categories/tests/', 2, 3, 'entry_list', 2)
        self.assertTemplateUsed(
            response, 'events/category/tests/entry_list.html')
    ...
    def test_zinnia_author_list(self):
        self.inhibit_templates('events/author_list.html')
    ...
    def test_zinnia_author_detail(self):
        self.inhibit_templates('events/author/admin/entry_list.html')
        response = self.check_publishing_context(
            '/authors/admin/', 2, 3, 'entry_list', 2)
        self.assertTemplateUsed(
            response, 'events/author/admin/entry_list.html')
    ...
    def test_zinnia_tag_list(self):
        self.inhibit_templates('events/tag_list.html')
        self.check_publishing_context(
            '/tags/', 1,
    ...
    def test_zinnia_tag_detail(self):
        self.inhibit_templates('events/tag/tests/entry_list.html', '404.html')
        response = self.check_publishing_context(
            '/tags/tests/', 2, 3, 'entry_list', 2)
        self.assertTemplateUsed(
            response, 'events/tag/tests/entry_list.html')
    ...
    def test_zinnia_entry_search(self):
        self.inhibit_templates('events/entry_search.html')
        self.check_publishing_context(
    ...
    def test_zinnia_sitemap(self):
        self.inhibit_templates('events/sitemap.html')
    ...
    def test_zinnia_trackback_on_entry_without_author(self):
        ...
        try:
            ...
            pass
        self.inhibit_templates('events/entry_trackback.xml')
    ...
    def test_capabilities(self):
        self.inhibit_templates(
            'events/humans.txt',
            'events/rsd.xml',
            'events/wlwmanifest.xml',
            'events/opensearch.xml')
