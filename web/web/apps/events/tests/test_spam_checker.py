...
from events.spam_checker import get_spam_checker
from events.spam_checker.backends.all_is_spam import backend
...
class SpamCheckerTestCase(TestCase):
    ...
    def test_get_spam_checker(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            self.assertEqual(
                get_spam_checker(
                    'events.tests.implementations.custom_spam_checker'), None)
            self.assertTrue(issubclass(w[-1].category, RuntimeWarning))
            self.assertEqual(
                str(w[-1].message),
        ...
        self.assertEqual(
            get_spam_checker('events.spam_checker.backends.all_is_spam'),
            backend)
