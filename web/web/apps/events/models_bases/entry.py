...
from events.markups import textile
from events.markups import markdown
from events.markups import restructuredtext
from events.preview import HTMLPreview
from events.flags import PINGBACK, TRACKBACK
from events.settings import UPLOAD_TO
from events.settings import MARKUP_LANGUAGE
from events.settings import ENTRY_DETAIL_TEMPLATES
from events.settings import ENTRY_CONTENT_TEMPLATES
from events.settings import AUTO_CLOSE_COMMENTS_AFTER
from events.settings import AUTO_CLOSE_PINGBACKS_AFTER
from events.settings import AUTO_CLOSE_TRACKBACKS_AFTER
from events.managers import entries_published
from events.managers import EntryPublishedManager
from events.managers import DRAFT, HIDDEN, PUBLISHED
from events.managers import CHINESE, ENGLISH, KOREAN, JAPANESE, RUSSIAN, TURKISH, SPANISH, FRENCH, GERMAN, ARABIC, NETHERLAND,FINNISH,INDONESIAN,ITALY,THAILAND,PORTUGUESE,VIETNAMESE,ROMANIA
from events.managers import TYPE_EVENTS, SUB_TYPE_ACTIVITY
from events.url_shortener import get_url_shortener
...
class CoreEntry(models.Model):
    ...
    ENTRY_TYPE_CHOICES = (
        (TYPE_EVENTS, _('Events')),
    )

    ENTRY_SUB_TYPE_CHOICES = (
        (SUB_TYPE_ACTIVITY, _('Events')),
    )
    ...
    language = models.IntegerField(
        _('Language'),
        choices=django_settings.LANGUAGE_CHOICES,default=codes.EntryLanguage.CHINESE.value,
        db_index=True, 
    ...
    entry_type = models.IntegerField(
        _('entry type'),
        choices=ENTRY_TYPE_CHOICES,default=TYPE_EVENTS,
        db_index=True,
...
class ContentTemplateEntry(models.Model):
    ...
    content_template = models.CharField(
        ...
        default='events/_entry_detail.html',
        choices=[('events/_entry_detail.html', _('Default template'))] +
