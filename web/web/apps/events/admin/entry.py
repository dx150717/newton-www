...
from events.managers import HIDDEN
from events.managers import PUBLISHED
from events.managers import TYPE_BLOG,TYPE_ANNOUNCEMENT,TYPE_COMMUNITY_VOICE
from events.settings import PROTOCOL
from events.models.author import Author
from events.ping import DirectoryPinger
from events.admin.forms import EntryAdminForm
from events.admin.filters import AuthorListFilter
from events.admin.filters import CategoryListFilter
...
class EntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_('Content'), {
            'fields': (('title', 'status'), 'content', 'image', 'event_date', 'event_during', 'event_summary', 'event_link')}),
        (_('Publication'), {
        ...
        (_('Categories'), {
            'fields': ('categories',),
            'classes': ('collapse', 'collapse-closed')}),
        (None, {'fields': ('tags', 'slug', 'language','entry_type','entry_sub_type','show_in_home')}))
    ...
    def get_tags(self, entry):
        ...
        try:
            return ', '.join(['<a href="%s" target="blank">%s</a>' %
                              (reverse('events:tag_detail', args=[tag]), tag)
    ...
    def get_sites(self, entry):
        ...
        """
        try:
            index_url = reverse('events:entry_archive_index')
        except NoReverseMatch:
            index_url = ''
        return ', '.join(
    ...
    def save_model(self, request, entry, form, change):
        ...
        if entry.pk and not request.user.has_perm('events.can_change_author'):
    ...
    def get_queryset(self, request):
        ...
        """
        if not request.user.has_perm('events.can_view_all'):
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        if db_field.name == 'authors':
            if request.user.has_perm('events.can_change_author'):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        readonly_fields = super(EntryAdmin, self).get_readonly_fields(
            request, obj)
        if not request.user.has_perm('events.can_change_status'):
    ...
    def get_actions(self, request):
        ...
        if not actions:
            return actions
        if (not request.user.has_perm('events.can_change_author') or
                not request.user.has_perm('events.can_view_all')):
