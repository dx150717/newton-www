...
def allocate_btc_address():
    ...
    try:
        ...
        allocated_address = kyc_models.DistributionInfo.objects.filter(phase_id=settings.CURRENT_FUND_PHASE).values_list('receive_btc_address', flat=True)
...
def allocate_ela_address():
    ...
    try:
        ...
        allocated_address = kyc_models.DistributionInfo.objects.filter(phase_id=settings.CURRENT_FUND_PHASE).values_list('receive_ela_address', flat=True)
        avaiable_address = list(set(all_address).difference(set(allocated_address)))
        if not avaiable_address:
